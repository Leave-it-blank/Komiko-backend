...
const comicForm = useForm({
  ...
  desc: props.comic.description,
  isHidden: props.comic.isHidden,
  isMature: props.comic.isMature,
  isLocked: props.comic.isLocked,
  tags: props.comic_tags,
  author: props.comic.author_id,
  artist: props.comic.artist_id,
  publisher: props.comic.publisher_id,
  country: props.comic.country_id,
  thumb: "",
  status: props.comic.status,
  choice: props.comic.choice,
  type: props.comic.type,
});
function submit() {
  comicForm.clearErrors();
  console.log(comicForm);
  let url = route("comics_management.comics.edit.store", props.comic.id);
  comicForm.post(url);
}
const titleStack = ref(["Dashboard", "Comics", "Edit Comic"]);
</script>
                    ...
                    >
                      ...
                      <div>
                        <ul class="pl-4 text-xs text-gray-500">
                          <li>Minimum: 490 x 690 (pixels).</li>
